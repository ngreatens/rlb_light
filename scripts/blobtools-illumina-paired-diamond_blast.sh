...
blast_db=/project/fdwsru_fungal/Nick/databases/nt/nt
diamond_db=/project/fdwsru_fungal/Nick/databases/uniprot/reference_proteomes

module load  bwa_mem2/2.2.1 samtools/1.16.1
...
bwa-mem2 mem \
    ...
    $forward_reads \
    $reverse_reads |
samtools sort -@16 -O BAM -o ${input_fasta%.*}_mapped.bam -
...
blastn -db $blast_db \
       ...
       -evalue 1e-25 \
       -num_threads 40 \
       -out ${input_fasta%.fasta}.ncbi.blastn.out
...
BLASTN_HITS=${input_fasta%.fasta}.ncbi.blastn.out
